...
struct daily_filename_calculator
{
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        return fmt_lib::format(
            SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}-{:02d}.{:02d}.{:02d}{}"), basename, now_tm.tm_year + 1900, now_tm.tm_mon + 1, now_tm.tm_mday
                    , now_tm.tm_hour, now_tm.tm_min, now_tm.tm_sec, ext);
    }
};
...
#    else
        {
            filename_t new_fmt_filename = fmt::format(SPDLOG_FMT_RUNTIME(fmt_filename), now_tm);
            //new_fmt_filename += "_" + GetTimeString(GetNowTimeStamp());
            return new_fmt_filename;
        }
#    endif
#endif
    }
...
private:
    void init_filenames_q_()
    {
        ...
        while (filenames.size() < max_files_)
        {
            ...
            //now -= std::chrono::hours(TIME_SPLIT);
    ...
    log_clock::time_point next_rotation_tp_()
    {
        ...
        return {rotation_time + std::chrono::minutes(60)/*std::chrono::hours(TIME_SPLIT)*/};
    }
